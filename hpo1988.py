...
plugin         = Plugin()
addon          = xbmcaddon.Addon("plugin.video.tranhuyhoang ")
pluginrootpath = "plugin://plugin.video.tranhuyhoang "
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.tranhuyhoang /(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.tranhuyhoang (/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.tranhuyhoang " in item["path"]:
